 ...
 void connect_tcp_server(const char *host, unsigned short port)
 {
    ssize_t retval = 0;
    ...
    { 
        // 将主机IP地址从“点分⼗进制”字符串 转化为 标准格式（32位整数）
        printf("inet_pton failed!\r\n");
        goto do_cleanup;
    }
    // 尝试和⽬标主机建⽴连接，连接成功会返回0 ，失败返回 -1
    ...
    // 建⽴连接成功之后，这个TCP socket描述符 —— sockfd 就具有了 “连接状态”，发送、接收 对端都是 connect 参数指定的⽬标主机和端⼝
