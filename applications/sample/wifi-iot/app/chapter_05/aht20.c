...
static uint32_t AHT20_Write(uint8_t* buffer, uint32_t buffLen)
 {
    ...
    if (retval != IOT_SUCCESS) {
        DEBUG_printf("I2cRead() failed, %0X!\n", retval);
        return retval;
    }
    return IOT_SUCCESS;*/
...
// 读取温湿度值之前， ⾸先要看状态字的校准使能位Bit[3]是否为 1(通过发送0x71可以获取⼀个字节的状态字)，
// 如果不为1，要发送0xBE命令(初始化)，此命令参数有两个字节， 第⼀个字节为0x08，第⼆个字节为0x00。
